...
char SPDF_IP[] = "10.60.8.51"; // ip address
...
char STEXT_IP[] = "10.60.8.51"; // ip address
...
int write_logs(char send_ack, char *file_name, char *err_msg, char *success_msg)
{
    ...
    int log_fd = open("/home/damlet/Desktop/asp_project/smain_log.txt", O_CREAT | O_WRONLY, 0777);
...
int send_data_to_stext_spdf(char *file_data, char *file_size, char *destination_path, char *dp_size, int sd)
{
    ...
    if ((bytes_read = write(sd, file_data, atoi(file_size))) <= 0)
    {
        printf("Write Failed\n");
        return -1;
    }
...
// saves file to appropriate server
// returns -1 on error, 1 on success
int save_file(char *file_data, char *file_size, char *destination_path, char *dp_size)
{
    ...
    case 0:
        // for .c
        // save file in smain
        return make_dirs_save_file(destination_path, file_data);
        break;

    case 1:
        // for .pdf
        return send_data_to_stext_spdf(file_data, file_size, destination_path, dp_size, spdf_sd);
        break;

    case 2:
        // for .txt
        return send_data_to_stext_spdf(file_data, file_size, destination_path, dp_size, stext_sd);
        break;
...
char *agg_data(char *dp_size, char *destination_path)
{
    ...
    char *result_all;
    char *results[3];
    char *c_data, *pdf_data, *text_data;
    int c_size = 0, pdf_size = 0, text_size = 0;
    int result_size = 0;
...
int main(int argc, char *argv[])
{
    ...
    {
        printf("Usage:smain smain_port spdf_port stext_port\n");
    }
}