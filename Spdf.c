...
int write_logs(char send_ack, char *destination_path, char *err_msg, char *success_msg)
{
    ...
    int log_fd = open("/home/damlet/Desktop/asp_project/spdf_log.txt", O_CREAT | O_WRONLY, 0777);
...
int make_dirs_save_file(char *destination_path, char *file_data, int file_bytes_size_)
{
    char *path = "/home/";
    ...
    mkdir("spdf", 0777); // make stext first if does not exist
    if (chdir("spdf") == -1)
    {
        printf("Change Directory Failed\n");
        return -1;
    ...
    write(fd, file_data, file_bytes_size_);
    close(fd);
    free(user_dir);
    chdir(cwd);
    free(cwd);
...
void upload_file()
{
    ...
    if (make_dirs_save_file(destination_path, file_data, atoi(file_size)) == -1)
    {
        send_ack = '0';
        printf("Save Failed\n");
    }
